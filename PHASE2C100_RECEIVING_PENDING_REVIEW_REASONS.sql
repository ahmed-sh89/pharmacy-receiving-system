...
begin
 ...
 if v_w='RECEIVING' then
   ...
   if v_id is not null then
     update public.pharmflow_needs_review_v1
     set pending_quantity=pharmflow_needs_review_v1.pending_quantity+v_q,
         review_reason=v_reason,updated_at=now()
     where id=v_id
     returning pharmflow_needs_review_v1.pending_quantity,pharmflow_needs_review_v1.created_at
     into v_q,v_time;
 ...
 values(
   p_pharmacy_id,v_w,v_g,nullif(p_raw_barcode,''),nullif(p_order_id,''),nullif(p_order_name,''),
   v_q,p_expiry_month,p_expiry_year,p_worker_id,v_worker,nullif(p_device_id,''),v_s,auth.uid(),v_reason
 )
...
create or replace function public.list_pharmacy_needs_review(
 p_pharmacy_id uuid,p_workflow text default null,p_order_id text default null
)
returns table(
 review_id uuid,workflow text,gtin text,raw_barcode text,order_id text,order_name text,
 pending_quantity integer,expiry_month integer,expiry_year integer,worker_id uuid,
 captured_by_name text,device_id text,source text,created_at timestamptz,review_reason text
)
language plpgsql security definer set search_path=public,pg_temp as $$
begin
 if auth.uid() is null or not public.is_pharmacy_member(p_pharmacy_id) then raise exception 'Pharmacy access required'; end if;
 return query
 select r.id,r.workflow,r.gtin,coalesce(r.raw_barcode,''),coalesce(r.order_id,''),coalesce(r.order_name,''),
        r.pending_quantity,r.expiry_month,r.expiry_year,r.worker_id,coalesce(r.captured_by_name,''),
        coalesce(r.device_id,''),r.source,r.created_at,coalesce(r.review_reason,'UNKNOWN_GTIN')
 from public.pharmflow_needs_review_v1 r
...
grant execute on function public.save_pharmacy_needs_review(
 uuid,text,text,text,text,text,integer,integer,integer,uuid,text,text,text
) to authenticated;
